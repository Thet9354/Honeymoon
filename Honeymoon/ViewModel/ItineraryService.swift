...
/// Where an itinerary came from — lets the UI distinguish a real AI plan from
/// the deterministic fallback (e.g. to only "spend" a free preview on a real one).
enum ItinerarySource {
    case cache       // a previously generated AI plan, served from Firestore
    case generated   // freshly generated by Claude this call
    case fallback    // deterministic local plan (offline / error / no allowance)
}
...
final class ItineraryService: ObservableObject {
    ...
    /// Returns an itinerary for a destination plus its source. `forceRegenerate`
    /// bypasses the cache (used by the "Regenerate" action).
    func itinerary(
        for destination: Destination,
        preferences: TravelPreferences,
        startDate: Date? = nil,
        forceRegenerate: Bool = false
    ) async -> (itinerary: Itinerary, source: ItinerarySource) {
        ...
        if !forceRegenerate,
           let cached = await loadCached(destinationId: destination.id, signature: signature) {
            return (Itinerary(destination: destination, generated: cached), .cache)
        }
